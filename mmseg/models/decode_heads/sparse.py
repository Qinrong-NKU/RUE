...
class SparseResNet18(spconv.SparseModule):
    ...
    def generate_sparse_inputs(self, img, entropy):
        batch=img.size(0)
        mask = torch.logical_and(entropy>0.5, entropy<0.999).float()
        mask = mask.float()
        spatial_shape = mask.size()[2:]
        indices = torch.where(mask.squeeze(1) > 0)
        x = x.permute(0, 2, 3, 1)
        x = x[indices]
        indices = torch.stack(indices, dim=1)
        x = spconv.SparseConvTensor(x, indices.int(), spatial_shape, batch)
        return x

    def forward(self, x,entropy):
        outs = []
        x = generate_sparse_inputs(x, entropy)
        x = self.conv1(x)
...
def l_sparse_resnet18(inc, stride=8, pretrained=False):
    ...
    if pretrained:
        state_dict = torch.load('/home/xx/ISDNet-main/pre_trained/resnet18.pth')
...
if __name__ == "__main__":
    ...
    model=l_sparse_resnet18(inc=4, stride=8)
    # print(model)