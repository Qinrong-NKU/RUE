...
def simple_fuse3(feat, feat16, values,entropy):
    entropy=F.interpolate(entropy, (feat16.shape[2], feat16.shape[3]), mode='bilinear', align_corners=False)
    entropy = entropy / 1.946
    values=F.interpolate(values, (feat16.shape[2], feat16.shape[3]), mode='bilinear', align_corners=False)
    values=sigmoid_function(values)
    values = (entropy+values)*0.5
    ...
    feat = F.interpolate(feat, (feat16.shape[2], feat16.shape[3]), mode='bilinear', align_corners=False)
    out = (1-values) * feat + values*feat16 
    return out,loss_weights
...
class ISDHead(BaseCascadeDecodeHead):
    ...
    def forward(self, inputs, prev_output, train_flag=True):
        ...
        entropy_score = entropy_score / 1.946
        ...
        local_shallow_feat8, local_shallow_feat16 = self.stdc_net(inputs,level=3)
        ...
        local_shallow_feat16,lossweights = simple_fuse3(deep_feat, local_shallow_feat16,values, entropy)
        output = self.conv_seg_aux_16(local_shallow_feat16)
        if train_flag:
            # if(prev_output[2]>0.6):
            #     lossweights=torch.ones_like(lossweights)
